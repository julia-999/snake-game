...
public class PlayGame extends JFrame implements KeyListener {
	...
	/* ======================================= */
		...
		if (c == KeyEvent.VK_X) { // Exit program
			...
			if (delay > MIN_DELAY)
				delay -= DELAY_STEP;
		} else if (c == KeyEvent.VK_Z) // Move snake slower
					delay += DELAY_STEP;
		else if (c == KeyEvent.VK_B) // Debug
				printDebugInfo();
		else if (c == KeyEvent.VK_P) // Pause
				toWait = true;
		else {
			if (c == KeyEvent.VK_W) direction = "up";
			else if (c == KeyEvent.VK_S) direction = "down";
			else if (c == KeyEvent.VK_A) direction = "left";
			else if (c == KeyEvent.VK_D) direction = "right";
	...
	/* ========================= */ 		
			...
			appleTile = makeTile("pixel person.png");
